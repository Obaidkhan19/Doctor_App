...
/// [MyCustomRefreshIndicator] far enough to demonstrate that they want the app to
/// refresh. The returned [Future] must complete when the refresh operation is
/// finished.
///
/// Used by [MyCustomRefreshIndicator.onRefresh].
typedef RefreshCallback = Future<void> Function();

// The state machine moves through these modes only when the scrollable
// identified by scrollableKey has been scrolled to its min or max limit.
enum _MyCustomRefreshIndicatorMode {
...
/// Used to configure how [MyCustomRefreshIndicator] can be triggered.
...
///
/// The trigger mode is configured by [MyCustomRefreshIndicator.triggerMode].
///
/// {@tool dartpad}
/// This example shows how [MyCustomRefreshIndicator] can be triggered in different ways.
///
/// ** See code in examples/api/lib/material/refresh_indicator/refresh_indicator.0.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example shows how to trigger [MyCustomRefreshIndicator] in a nested scroll view using
...
///
/// The [MyCustomRefreshIndicator] will appear if its scrollable descendant can be
/// overscrolled, i.e. if the scrollable's content is bigger than its viewport.
/// To ensure that the [MyCustomRefreshIndicator] will always appear, even if the
...
/// ```
///
/// A [MyCustomRefreshIndicator] can only be used with a vertical scroll view.
///
/// See also:
///
///  * <https://material.io/design/platform-guidance/android-swipe-to-refresh.html>
///  * [RefreshIndicatorState], can be used to programmatically show the refresh indicator.
///  * [RefreshProgressIndicator], widget used by [MyCustomRefreshIndicator] to show
///    the inner circular progress spinner during refreshes.
///  * [CupertinoSliverRefreshControl], an iOS equivalent of the pull-to-refresh pattern.
///    Must be used as a sliver inside a [CustomScrollView] instead of wrapping
///    around a [ScrollView] because it's a part of the scrollable instead of
///    being overlaid on top of it.
class MyCustomRefreshIndicator extends StatefulWidget {
  ...
  const MyCustomRefreshIndicator.adaptive({
    super.key,
    required this.child,
    this.displacement = 40.0,
    this.edgeOffset = 0.0,
  ...
  /// Defines how this [MyCustomRefreshIndicator] can be triggered when users overscroll.
  ///
  /// The [MyCustomRefreshIndicator] can be pulled out in two cases,
  ...
  @override
  MyCustomRefreshIndicatorState createState() =>
      MyCustomRefreshIndicatorState();
}

/// Contains the state for a [MyCustomRefreshIndicator]. This class can be used to
/// programmatically show the refresh indicator, see the [show] method.
class MyCustomRefreshIndicatorState extends State<MyCustomRefreshIndicator>
    with TickerProviderStateMixin<MyCustomRefreshIndicator> {
  ...
  @override
  void didUpdateWidget(covariant MyCustomRefreshIndicator oldWidget) {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (_shouldStart(notification)) {
      setState(() {
        _mode = _MyCustomRefreshIndicatorMode.drag;
      });
      return false;
    }
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == _MyCustomRefreshIndicatorMode.drag ||
          _mode == _MyCustomRefreshIndicatorMode.armed) {
        _dismiss(_MyCustomRefreshIndicatorMode.canceled);
      }
      ...
      if (_mode == _MyCustomRefreshIndicatorMode.armed &&
          notification.dragDetails == null) {
      ...
      switch (_mode) {
        case _MyCustomRefreshIndicatorMode.armed:
          _show();
          break;

        case _MyCustomRefreshIndicatorMode.drag:
          _dismiss(_MyCustomRefreshIndicatorMode.canceled);
          break;
        case _MyCustomRefreshIndicatorMode.canceled:
        case _MyCustomRefreshIndicatorMode.done:
        case _MyCustomRefreshIndicatorMode.refresh:
        case _MyCustomRefreshIndicatorMode.snap:
        case null:
          // do nothing
          break;
      }
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == _MyCustomRefreshIndicatorMode.drag ||
        _mode == _MyCustomRefreshIndicatorMode.armed);
    double newValue =
  ...
  Future<void> _dismiss(_MyCustomRefreshIndicatorMode newMode) async {
    await Future<void>.value();
    // This can only be called from _show() when refreshing and
    // _handleScrollNotification in response to a ScrollEndNotification or
    // direction change.
    assert(newMode == _MyCustomRefreshIndicatorMode.canceled ||
        newMode == _MyCustomRefreshIndicatorMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode!) {
  ...
  void _show() {
    assert(_mode != _MyCustomRefreshIndicatorMode.refresh);
    assert(_mode != _MyCustomRefreshIndicatorMode.snap);
    final Completer<void> completer = Completer<void>();
    _pendingRefreshFuture = completer.future;
    _mode = _MyCustomRefreshIndicatorMode.snap;
    _positionController
        .animateTo(1.0 / _kDragSizeFactorLimit,
            duration: _kIndicatorSnapDuration)
        .then<void>((void value) {
      if (mounted && _mode == _MyCustomRefreshIndicatorMode.snap) {
        setState(() {
        ...
        refreshResult.whenComplete(() {
          if (mounted && _mode == _MyCustomRefreshIndicatorMode.refresh) {
            completer.complete();
            _dismiss(_MyCustomRefreshIndicatorMode.done);
          }
        });
      }
    });
  ...
  Future<void> show({bool atTop = true}) {
    if (_mode != _MyCustomRefreshIndicatorMode.refresh &&
        _mode != _MyCustomRefreshIndicatorMode.snap) {
      if (_mode == null) {
