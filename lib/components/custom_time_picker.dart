...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint selectorPaint = Paint()..color = ColorManager.kPrimaryColor;
...
class _Dial extends StatefulWidget {
  const _Dial({
    ...
    required this.CustomVisibleStep,
    required this.mode,
    required this.use24HourDials,
    required this.onChanged,
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  List<_TappableLabel> _buildMinutes(TextTheme textTheme, Color color) {
    ...
    for (int i = 0;
        i < (TimeOfDay.minutesPerHour / widget.CustomVisibleStep).ceil();
        i++) {
      minuteMarkerValues
          .add(TimeOfDay(hour: 0, minute: i * widget.CustomVisibleStep));
    }
...
class IntervalTimePickerDialog extends StatefulWidget {
  ...
  const IntervalTimePickerDialog({
    ...
    this.interval = 1,
    this.MyCustomVisibleStep = CustomVisibleStep.fifths,
    this.cancelText,
    this.confirmText,
    this.helpText,
  ...
  final CustomVisibleStep MyCustomVisibleStep;
...
class _IntervalTimePickerDialogState extends State<IntervalTimePickerDialog>
  ...
  late final int _CustomVisibleStep;
  ...
  void initState() {
    ...
    _CustomVisibleStep = _parseVisibleStep(widget.MyCustomVisibleStep);
  }

  int _parseVisibleStep(CustomVisibleStep vS) {
    switch (vS) {
      case CustomVisibleStep.fifths:
        return 5;
      case CustomVisibleStep.sixths:
        return 6;
      case CustomVisibleStep.tenths:
        return 10;
      case CustomVisibleStep.twelfths:
        return 12;
      case CustomVisibleStep.fifteenths:
        return 15;
      case CustomVisibleStep.twentieths:
        return 20;
      case CustomVisibleStep.thirtieths:
        return 30;
      case CustomVisibleStep.sixtieth:
        return 60;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final MediaQueryData media = MediaQuery.of(context);
    final TimeOfDayFormat timeOfDayFormat =
        localizations.timeOfDayFormat(alwaysUse24HourFormat: true);
    final bool use24HourDials = hourFormat(of: timeOfDayFormat) != HourFormat.h;
    final ThemeData theme = Theme.of(context);
        ...
        final Widget dial = Padding(
          ...
          child: ExcludeSemantics(
            child: AspectRatio(
              ...
              child: _Dial(
                interval: _interval,
                CustomVisibleStep: _CustomVisibleStep,
                mode: _mode.value,
                use24HourDials: use24HourDials,
                selectedTime: _selectedTime.value,
...
///
/// The [CustomVisibleStep] parameter is used for which minute labels the TimePicker will show.
///
...
Future<TimeOfDay?> showCustomIntervalTimePicker({
  required BuildContext context,
  required TimeOfDay initialTime,
  int interval = 1,
  CustomVisibleStep CustomVisibleStep = CustomVisibleStep.fifths,
  ...
  final Widget dialog = IntervalTimePickerDialog(
    ...
    interval: interval,
    MyCustomVisibleStep: CustomVisibleStep,
